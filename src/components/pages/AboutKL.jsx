...
import image2 from "../../assets/images/partners/telangana.png";
        ...
        <div className="container-page">
          ...
          >
            ...
            <div className="partner-content"  style={{marginBottom:"44px"}}>
              The platform is facilitated by Government of Telangana and UNDP,
            ...
            <div>
            {allLayers.map((item, index) => item.isavailable ?
          (
              <Accordion flush open={open} toggle={toggle}>
                <AccordionItem style={{background: "#091b33"}}>
                  <AccordionHeader targetId={index + "1"} className="about-accordion-heading">{item.display_name}</AccordionHeader>
                  <AccordionBody accordionId={index + "1"} className="about-accordion-content">
                  <div className="about-layer">
                  <div className="layer-heading" style={{marginBottom:"25px"}}>Description</div>
                  <div className="layer-description" style={{marginBottom:"25px"}}>{item.long_description}</div>
                  <div className="layer-heading" style={{marginBottom:"25px"}}>Source</div>
                  <div className="layer-description" style={{marginBottom:"25px"}}>{item.source}</div>
                  <div className="layer-heading" style={{marginBottom:"25px"}}>Citation</div>
                  <div className="layer-description" style={{marginBottom:"25px"}}>{item.citation}</div>
                  <div className="layer-heading" style={{marginBottom:"25px"}}>Standards</div>
                  <div className="layer-description">{item.standards}</div>
                  </div>
                  </AccordionBody>
                </AccordionItem>
              </Accordion>
